	...
	where $\tau$ is the fixed proportional tax rate, $T_t$ is a residual lump-sum tax (or transfer) that balances the government budget dynamically, $w_t$ the real wage, $R_t$ the rental rate on $K_{p,t-1}$, $\Pi_t$ the rents from the exogenous energy endowment, and $B^*_t$ net foreign assets.\footnote{\label{fn:timing_convention}Capital installed at $t-1$ is productive in $t$; investment $I_{p,t}$ does not contribute until $t+1$.}
...
\subsection{Government} \label{sec:government}
	...
	The government finances grid investment through a fixed proportional tax $\tau_{ss}=I_{grid,ss}/Y_{ss}=1.43\%$ ($\tau_{ss}=\delta_g K_{g,ss}/Y_{ss}=0.0143$), ensuring tax revenue covers replacement grid investment. Formally, the government budget is balanced dynamically by a lump-sum transfer $T_t$:
	\begin{equation}
