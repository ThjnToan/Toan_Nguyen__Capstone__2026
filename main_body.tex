	...
	where $\tau$ is the fixed proportional tax, $T_t$ is a residual lump-sum tax (or transfer) that balances the government budget dynamically ($I_{grid,t} = \tau Y_t + T_t$), $w_t$ the real wage, $R_t$ the rental rate on $K_{p,t-1}$, $\Pi_t$ the rents from the exogenous energy endowment, and $B^*_t$ net foreign assets.\footnote{\label{fn:timing_convention}Capital installed at $t-1$ is productive in $t$; investment $I_{p,t}$ does not contribute until $t+1$.}
...
The recursive competitive equilibrium is formally stated in Section~\ref{sec:equilibrium}; the transmission channels above constitute its key characterisation. The Blanchard-Kahn conditions are satisfied with 3 unstable eigenvalues matching the 3 forward-looking variables.
	...
	\begin{table}[H]
		...
		\begin{tabular}{l c c c p{3.5cm}}
			...
			Battery/grid capital ratio & $\approx$0.14--0.16 & 0.167 & F & \citep{mof2023} \\
			\bottomrule
		\end{tabular}
		\vspace{0.2cm}
	...
	\paragraph{Current-account monotonicity.} The current-account surplus shrinks monotonically with $\phi_b$ ($+0.048$ to $+0.003$ to $+0.001$), consistent with Vietnam's observed modest surpluses during domestic slowdowns \citep{worldbank2024}.
