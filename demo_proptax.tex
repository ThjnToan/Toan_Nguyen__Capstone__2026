...
Calibrating to Vietnam's Power Development Plan 8, I find five results. First, intermittency shocks account for 97.7\% of output volatility and 95.8\% of reliability volatility, with global battery-price shocks contributing the remainder through the fiscal amplification channel. Second, global battery-price shocks drive 100\% of battery investment volatility, highlighting exposure to external supply chains. Third, scarcity-induced technological change generates persistent battery improvements conditional on regulatory transmission of scarcity signals ($\chi$). Fourth, proportional taxation creates a fiscal amplification channel: a one-standard-deviation intermittency shock reduces output by 0.59\% and grid utilization by 1.30\%, while the tax rate overshoots by 2.54\% relative to steady state. Fifth, suppressing scarcity signals ($\chi = 0$) raises the welfare cost of intermittency by 27\%, demonstrating that market mechanisms are quantitatively important complements to storage investment.
...
\paragraph{Euler equation for capital.} With proportional taxation, the after-tax return to capital in period $t+1$ is $(1-\tau_{t+1})\alpha Y_{t+1}/K_{p,t+1}$. The intertemporal optimality condition becomes:\footnote{The full step-by-step Lagrangian derivation of this condition is provided in Appendix~\ref{app:deriv_euler}.}
\begin{equation}
\frac{1}{C_t} = \beta \mathbb{E}_t \left[ \frac{(1-\tau_{t+1})\,\alpha Y_{t+1}/K_{p,t+1} + 1 - \delta_p}{C_{t+1}} \right]
\label{eq:euler}
\end{equation}

The household invests in capital until the marginal cost of saving today equals the expected discounted after-tax return. When an intermittency shock raises $\tau_{t+1}$ (via higher grid investment), it reduces the after-tax return to capital, amplifying the contractionary effect on investment beyond the direct utilization channel.
...
The after-tax labor share of value-added (left side) equals the household's required compensation for working. The proportional tax $\tau_t$ drives a wedge that reduces effective labor income, lowering equilibrium labor supply relative to the lump-sum benchmark. The parameter $\varphi$ is calibrated endogenously to match steady-state labor hours of $L_{ss} = 0.33$.
...
The government runs a balanced budget each period, financing grid investment through a proportional tax $\tau_t$ on household output income. The tax rate adjusts endogenously each period to balance the budget:
\begin{equation}
\tau_t = \frac{I_{grid,t}}{Y_t}
\label{eq:gov_budget}
\end{equation}

\noindent so that total tax revenue $\tau_t Y_t = I_{grid,t}$ exactly covers grid investment spending. This proportional tax is economically motivated by Vietnam's practice of financing EVN grid expenditures through regulated electricity tariffs, which act as a charge roughly proportional to economic activity. Unlike a lump-sum tax, $\tau_t$ is distortionary: it drives a wedge between the household's gross and net income, reducing the effective return to labor and capital and thereby affecting equilibrium allocations. Crucially, since $I_{grid,t}$ responds to the reliability gap (equation~\ref{eq:grid_investment_rule}), the tax rate $\tau_t$ rises endogenously when intermittency worsens --- creating a \textit{fiscal amplification channel}: a negative reliability shock not only reduces capital utilization directly, but also raises the tax burden, further crowding out private consumption and investment.
...
Substituting the government budget constraint \eqref{eq:gov_budget} ($\tau_t Y_t = I_{grid,t}$) into the household budget constraint \eqref{eq:hh_budget} eliminates $\tau_t Y_t$ and yields the economy-wide resource constraint. Unlike the lump-sum case, the proportional tax does \textit{not} satisfy Ricardian equivalence: households optimise over after-tax income $(1-\tau_t)Y_t$, so the tax distorts equilibrium labour supply and investment decisions. Nevertheless, the consolidated resource constraint is identical in form --- the tax revenue and government spending cancel out --- because the government runs a balanced budget period by period. On the income side, substituting $w_t L_t + R_t K_{p,t-1} = V_t$ and noting that $\omega_E = 0.045$ is small so $V_t \approx Y_t$, the economy-wide resource constraint in the open economy is:
\begin{equation}
...
\begin{enumerate}
    ...
    \item Proportional tax rate: $\tau_{ss} = I_{grid,ss}/Y_{ss} = \delta_g K_{g,ss}/Y_{ss} \approx 0.0165$ (1.65\%). This endogenous tax rate is small but distortionary, driving a wedge between gross and net factor returns.
...
\paragraph{Channel 4: Fiscal Amplification (Proportional Tax Variant).}
This channel is unique to the proportional tax specification. When an intermittency shock hits, grid investment $I_{grid,t}$ rises (reliability gap widens the investment rule) while output $Y_t$ falls (reliability penalty). Because $\tau_t = I_{grid,t}/Y_t$, both forces raise the tax rate simultaneously---the numerator rises and the denominator falls. This elevated $\tau_t$ then distorts two equilibrium conditions: (i) it lowers the after-tax return on capital in the Euler equation, crowding out productive investment; and (ii) it reduces the after-tax real wage in the labor supply condition, contracting labor supply and value-added. The result is an endogenous fiscal amplification of the original intermittency shock that is absent under lump-sum taxation.
...
\begin{figure}[H]
    ...
    \parbox{0.95\textwidth}{\footnotesize \textit{Note:} Impulse responses to a one standard deviation intermittency shock ($\sigma_{ren} = 0.12$). Left axis (blue): output $\hat{Y}_t$ and productive investment $\hat{I}_{p,t}$ in percent deviations from steady state. Right axis (red): proportional tax rate deviation $\hat{\tau}_t$. The tax rate rises 2.54\% relative to steady state on impact (a 0.042 pp increase from $\tau_{ss}=1.65\%$) as grid investment surges while output falls, creating an after-tax cost wedge that amplifies the contraction in productive investment. This fiscal amplification channel is unique to the proportional tax specification and absent under lump-sum taxation.}
\end{figure}
...
The model's central qualitative contribution emerges in the investment dynamics. Both battery investment and grid investment respond positively to the intermittency shock, as the reliability gap triggers the investment rules. Battery investment increases by 1.95\% relative to its steady-state level on impact, while grid investment rises by 1.95\%. While both increase proportionally by the same amount, grid investment rises more in absolute level (0.000278 vs.\ 0.000111 in model units), yielding an agility ratio of $I_{bat}/I_{grid} = 0.40$: batteries absorb only 40\% as much absolute investment as grid infrastructure on impact. The key insight is the structural mechanism: grid investment is fiscally tied to output through $\tau_t = I_{grid,t}/Y_t$, while battery investment responds to reliability through the private investment rule.
...
Grid investment, by contrast, operates through the proportional tax rule and achieves the same 0.540\% impact response, as the proportional tax mechanism channels both reliability scarcity and fiscal constraints into symmetric flexibility investment:
\begin{equation}
...
The proportional tax $\tau_t = I_{grid,t}/Y_t$ introduces a \textbf{fiscal amplification channel} absent in lump-sum specifications. When an intermittency shock hits, $I_{grid,t}$ rises (+1.95\% of SS) while $Y_t$ falls (−0.59\% of SS), causing $\tau_t$ to rise 2.54\% relative to steady state on impact (a 0.042 percentage point increase). This higher after-tax cost wedge simultaneously suppresses productive investment (−0.057\%) while battery and grid investment each surge by 1.95\%. The result is a modest but structural reallocation from private productive capital to flexibility investment---the model's fiscal amplification mechanism.

The equal proportional response of battery and grid investment (1.95\% each) masks an important asymmetry in levels: grid investment rises 2.5$\times$ more in absolute units due to its larger steady-state base. Under proportional taxation, the fiscal channel creates a critical macroeconomic tradeoff: higher grid investment raises $\tau$, which reduces the after-tax marginal product of capital and wage, lowering investment incentives. Market-based pricing mechanisms---such as scarcity pricing, capacity markets, or ancillary service contracts---could decouple the investment response from the fiscal channel, improving welfare outcomes.

\subsubsection{Stage 3: Scarcity-Induced Innovation Activation}

The reliability gap also activates the model's scarcity-induced technological change mechanism. By quarter 10, battery technology ($A_{bat}$) has improved by approximately 0.117\% above the baseline trajectory. This improvement represents productivity-enhancing innovation in storage: better battery management systems, optimized charging algorithms, and engineering advances induced by the persistent scarcity signal. Note that because $F_t$ uses $A_{bat,t-1}$, the technology improvement induced at quarter $t$ only raises flexibility from quarter $t+1$ onward---reflecting the one-period delay between the innovation decision and its operational impact.
...
Consumption falls by 0.069\% on impact. Unlike the lump-sum specification, the proportional tax creates an after-tax income wedge that prevents standard consumption smoothing. More importantly, the AR(1) persistence of the intermittency shock ($\rho_{ren}=0.85$) means households anticipate prolonged reliability shortfalls, inducing a larger precautionary reduction in current consumption. The output decline (0.59\%) is substantially transmitted to consumption, while productive investment also falls by 0.057\% and international borrowing partially absorbs the shock ($B^*$ falls 0.61\% of its SS value on impact). The relatively large consumption response (compared to the i.i.d. benchmark) reflects the forward-looking channel: persistent shocks generate larger intertemporal distortions.
...
\begin{table}[H]
    ...
    \end{tabular}
    \vspace{0.2cm}
    \parbox{\textwidth}{\footnotesize \textit{Note:} Percentage of forecast error variance attributable to each shock from Dynare stochastic simulation (1000 periods). Shares can exceed 100\% due to non-zero correlation of simulated shocks in finite samples. Intermittency shocks drive 97.7\% of output volatility; global battery-price shocks explain only 4\% through the fiscal amplification channel. Consumption volatility is split between intermittency (77\%) and battery-price shocks (12\%), reflecting the dominance of persistent domestic shocks. The tax rate inherits 68\% of its variance from grid implementation shocks ($\tau_t = I_{grid,t}/Y_t$) and 41\% from intermittency.}
\end{table}

Four findings emerge. First, intermittency shocks drive 97.7\% of output volatility, with battery-price shocks contributing only 4\% through the proportional tax's fiscal amplification channel---a distinctive feature absent in lump-sum models. The dominance of intermittency reflects the AR(1) persistence ($\rho_{ren}=0.85$): once reliability degrades, the deficiency propagates across many quarters, generating large cumulative forecast errors. Second, utilization volatility is dominated by intermittency (95.8\%), with battery-price shocks explaining only 5\% through capital accumulation dynamics. Third, battery investment volatility is almost entirely driven by global price shocks (100\%), confirming Vietnam's price-taking position in world battery markets. Fourth, consumption volatility is split between intermittency (77\%) and battery-price shocks (12\%): unlike the i.i.d. case, persistent intermittency shocks propagate strongly into household consumption through the reliability and fiscal channels. The proportional tax rate inherits 68\% of its variance from grid implementation shocks, as $\tau_t = I_{grid,t}/Y_t$ directly amplifies any government investment disturbance into a fiscal shock on households, with an additional 41\% from intermittency shocks.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c l}
        ...
        \midrule
        Output ($Y$) & $-0.59$ & 97.7\% driven by intermittency \\
        Utilization ($u$) & $-1.30$ & 95.8\% driven by intermittency \\
        Consumption ($C$) & $-0.069$ & Persistent shock amplifies forward-looking consumption drop \\
        ...
        Tax rate ($\tau$) & $+2.54$ & Fiscal amplification channel: $\hat{\tau} = \hat{I}_{grid} - \hat{Y}$ \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
    \parbox{\textwidth}{\footnotesize \textit{Note:} All values are percentage deviations from steady state on impact ($t=0$) following a one standard deviation intermittency shock ($\sigma_{ren} = 0.12$). Computed from Dynare first-order perturbation of \texttt{vietnam\_dsge\_proptax.mod}. Variance shares from simulation-based FEVD (Table~\ref{tab:fevd}). The proportional tax rate rise of 2.54\% (a 0.042 pp increase from $\tau_{ss}=1.65\%$) reflects the fiscal amplification channel $\hat{\tau}_t = \hat{I}_{grid,t} - \hat{Y}_t$. The deterministic steady state (Table~\ref{tab:steady_state}) serves as the reference economy for the welfare comparisons in Section~\ref{sec:welfare}.}
\end{table}
...
This baseline estimate should be interpreted as a lower bound for a single shock. The model omits amplifying factors (distributional impacts, industrial hysteresis, political instability) and dampening factors (household adaptation, demand response, regional grid interconnections). The $\Lambda_{\text{baseline}} = 0.000402\%$ serves as the reference point for the counterfactual analysis below, where I examine how regulatory distortions and structural parameters alter this cost.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c}
        ...
        \midrule
        Full transmission ($\chi = 1.0$) & 0.000402\% & --- (baseline) \\
        Partial dampening ($\chi = 0.5$) & 0.000451\% & $+$12\% \\
        Partial dampening ($\chi = 0.3$) & 0.000473\% & $+$18\% \\
        Complete suppression ($\chi = 0.0$) & 0.000511\% & $+$27\% \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
...
The results demonstrate that signal attenuation has quantitatively important welfare consequences. Moving from full transmission to complete suppression increases the welfare cost of intermittency by 27\%, from 0.000402\% to 0.000511\% of steady-state consumption. Even partial dampening ($\chi = 0.3$, representing regulated markets with incomplete ancillary service remuneration) raises the cost by 18\%. This monotonic increase arises because the scarcity-induced innovation channel is progressively deactivated as $\chi$ falls: battery technology fails to improve in response to reliability shortfalls, forcing the economy to rely solely on costly capital accumulation to restore grid stability.
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c c c}
        ...
        \midrule
        0.5 & 0.000402\% & $+$0.0001 & $+$0.0001 & $-$0.063 \\
        1.0 & 0.000402\% & $+$0.0002 & $+$0.0001 & $-$0.061 \\
        1.5 (baseline) & 0.000402\% & $+$0.0003 & $+$0.0001 & $-$0.059 \\
        2.0 & 0.000402\% & $+$0.0004 & $+$0.0001 & $-$0.058 \\
        3.0 & 0.000403\% & $+$0.0006 & $+$0.0001 & $-$0.054 \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c c c}
        ...
        \midrule
        0.980 & $\sim 8.2\%$ & 0.000506\% & $-0.0051$ & 78.2 \\
        0.990 (baseline) & $\sim 4.0\%$ & 0.000402\% & $-0.0050$ & 72.8 \\
        0.995 & $\sim 2.0\%$ & 0.000357\% & $-0.0049$ & 68.7 \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
...
\paragraph{Mechanism 2: Battery Price Shock $\to$ Terms of Trade.} A positive shock to $P_{bat}$ operates like an adverse terms-of-trade shock for a battery-importing economy. Higher import prices for storage technology raise the cost of flexibility investment, slow capital accumulation in $K_b$, and---through the reliability channel---reduce output. The variance decomposition confirms the quantitative importance of this channel: battery price shocks account for approximately 79\% of $B^*$ volatility, while intermittency now dominates consumption volatility (77\%) with battery-price shocks contributing 12\%, reflecting the larger persistent impact of domestic reliability shocks relative to the i.i.d.\ benchmark.

\paragraph{Dual Exposure.} The variance decomposition of net foreign assets reveals the economy's dual exposure: intermittency shocks contribute 43\% and battery price shocks contribute 79\% of $B^*$ volatility (shares exceed 100\% due to negative covariance between shock contributions). This pattern has a clear economic interpretation: domestic reliability risk creates borrowing needs for consumption smoothing and investment financing, while global battery market shocks determine the cost of the flexibility imports required to close the reliability gap. The debt-elastic interest rate premium ($r^*_t = \bar{r} + \phi_b(e^{\bar{B}^* - B^*_t} - 1)$) ensures that persistent borrowing raises the country's risk premium, providing a stabilizing feedback that prevents explosive debt accumulation but amplifies the welfare cost of repeated shocks.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c}
        ...
        \midrule
        Output ($Y$) & $-0.0051\%$ & $+0.0001\%$ & $-0.0050\%$ \\
        Consumption ($C$) & $-0.0004\%$ & $-0.0002\%$ & $-0.0006\%$ \\
        Reliability ($u$) & $-0.0126\%$ & $+0.0000\%$ & $-0.0126\%$ \\
        Battery Investment ($I_{bat}$) & $+0.0001\%$ & $-0.0005\%$ & $-0.0003\%$ \\
        Net Foreign Assets ($B^*$) & $-0.0524\%$ & $+0.0021\%$ & $-0.0523\%$ \\
        \midrule
        Welfare cost ($\Lambda$) & $0.000402\%$ & $0.000155\%$ & $0.000557\%$ \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
...
Three findings emerge from the joint scenario. First, the welfare costs are additive: the joint welfare cost (0.000557\%) essentially equals the sum of individual costs (0.000402\% $+$ 0.000155\% = 0.000557\%), reflecting the linear superposition property of the first-order perturbation solution. Under higher-order approximations, nonlinear interaction effects could generate amplification.

Second, intermittency now dominates the welfare cost---contributing more than twice the battery price component (0.000402\% vs.\ 0.000155\%). This reverses the finding from i.i.d.\ shock specifications and reflects the key role of AR(1) persistence ($\rho_{ren}=0.85$): once reliability degrades, households suffer prolonged consumption losses, generating a much larger welfare cost than a purely transitory disruption.
...
\textbf{First, the reliability penalty is a tangible macroeconomic friction, amplified by fiscal feedback.} A one-standard-deviation intermittency shock reduces grid utilization by 1.30\% and output by 0.59\% through the effective capital channel, with the proportional tax adding a fiscal amplification wedge of 2.54\% relative to steady state. Variance decomposition confirms that intermittency shocks account for 97.7\% of output volatility, with battery-price shocks contributing the remaining 4\% through the fiscal channel. The dominance of intermittency reflects its AR(1) persistence ($\rho_{ren}=0.85$), which generates sustained multi-quarter reliability degradation. Unlike standard productivity shocks, this penalty is structural and predictable---it arises from the physics of the grid combined with the fiscal response rule, not from exogenous technological regress.
...
First, the \textit{reliability penalty with fiscal amplification}: intermittency shocks reduce grid utilization by 1.30\%, generating output losses of 0.59\%, and trigger a 2.54\% overshoot in the proportional tax rate that further amplifies the contraction. Variance decomposition confirms that intermittency shocks account for 97.7\% of output volatility, with the remaining 4\% attributable to global battery-price shocks propagating through the fiscal amplification channel.
...
The welfare cost of intermittency is estimated at 0.000402\% of steady-state consumption under full price signal transmission (from Dynare first-order perturbation). Counterfactual analysis reveals that this cost increases by 27\% when regulatory distortions fully suppress scarcity signals ($\chi = 0$), demonstrating that the scarcity-induced innovation channel is quantitatively important for welfare. The fiscal amplification channel introduced by proportional taxation creates a critical macroeconomic tradeoff: grid investment surges reduce future productive capacity, creating a welfare cost that depends critically on the degree of scarcity signal transmission through the innovation mechanism.
...
This is the modified capital Euler equation~\eqref{eq:euler} under proportional taxation. Compared to the lump-sum case, the after-tax return to capital is $(1-\tau_{t+1})\alpha Y_{t+1}/K_{p,t+1}$ rather than $\alpha Y_{t+1}/K_{p,t+1}$. Since $\tau_{t+1}$ rises when intermittency worsens (via the grid investment rule), a reliability shock lowers the expected after-tax return to capital, reducing investment beyond the direct utilization effect. This is the fiscal amplification channel.
