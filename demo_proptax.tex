...
\begin{table}[H]
    ...
    \end{tabular}
    \vspace{0.2cm}
    \parbox{\textwidth}{\footnotesize \textit{Note:} Percentage of forecast error variance attributable to each shock from Dynare stochastic simulation (1000 periods). Shares can exceed 100\% due to non-zero correlation of simulated shocks in finite samples. Intermittency shocks drive 96.8\% of output volatility; global battery-price shocks contribute 4\%. Consumption volatility is split between intermittency (77\%) and battery-price shocks (12\%), reflecting the dominance of persistent domestic shocks. The proportional tax rate $\tau$ is a fixed parameter and does not appear in the FEVD. Grid investment variance is 82\% driven by implementation shocks $\varepsilon_I$ via the investment rule.}
\end{table}

Four findings emerge. First, intermittency shocks drive 96.8\% of output volatility, with battery-price shocks contributing only 4\%. The dominance of intermittency reflects the AR(1) persistence ($\rho_{ren}=0.85$): once reliability degrades, the deficiency propagates across many quarters, generating large cumulative forecast errors. Second, utilization volatility is dominated by intermittency (95.8\%), with battery-price shocks explaining only 5\% through capital accumulation dynamics. Third, battery investment volatility is almost entirely driven by global price shocks (100\%), confirming Vietnam's price-taking position in world battery markets. Fourth, consumption volatility is split between intermittency (77\%) and battery-price shocks (12\%): persistent intermittency shocks propagate strongly into household consumption through the reliability channel. Since the proportional tax rate $\tau$ is fixed, it does not contribute as a variance source; grid investment variance is 82\% determined by implementation shocks $\varepsilon_I$ acting directly through the investment rule.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c l}
        ...
        \midrule
        Output ($Y$) & $-0.55$ & 96.8\% driven by intermittency \\
        ...
        Productive investment ($I_p$) & $-23.5$ & Euler amplification: fall in future $Y$ compresses after-tax MPK \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
...
\begin{equation*}
\Lambda_{\text{baseline}} = 0.000386\% \text{ of steady-state consumption}
\end{equation*}
...
This baseline estimate should be interpreted as a lower bound for a single shock. The model omits amplifying factors (distributional impacts, industrial hysteresis, political instability) and dampening factors (household adaptation, demand response, regional grid interconnections). The $\Lambda_{\text{baseline}} = 0.000386\%$ serves as the reference point for the counterfactual analysis below, where I examine how regulatory distortions and structural parameters alter this cost.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c}
        ...
        \midrule
        Full transmission ($\chi = 1.0$) & 0.000386\% & --- (baseline) \\
        Partial dampening ($\chi = 0.5$) & 0.000434\% & $+$12\% \\
        Partial dampening ($\chi = 0.3$) & 0.000456\% & $+$18\% \\
        Complete suppression ($\chi = 0.0$) & 0.000492\% & $+$27\% \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c c c}
        ...
        \midrule
        0.5 & 0.000386\% & $+$0.0001 & $+$0.0001 & $-$0.063 \\
        1.0 & 0.000386\% & $+$0.0002 & $+$0.0001 & $-$0.061 \\
        1.5 (baseline) & 0.000386\% & $+$0.0003 & $+$0.0001 & $-$0.059 \\
        2.0 & 0.000387\% & $+$0.0004 & $+$0.0001 & $-$0.058 \\
        3.0 & 0.000387\% & $+$0.0006 & $+$0.0001 & $-$0.054 \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c c c}
        ...
        \midrule
        0.980 & $\sim 8.2\%$ & 0.000486\% & $-0.0051$ & 78.2 \\
        0.990 (baseline) & $\sim 4.0\%$ & 0.000386\% & $-0.0050$ & 73.2 \\
        0.995 & $\sim 2.0\%$ & 0.000343\% & $-0.0049$ & 68.7 \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c}
        ...
        \midrule
        Welfare cost ($\Lambda$) & $0.000386\%$ & $0.000151\%$ & $0.000537\%$ \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
...
Three findings emerge from the joint scenario. First, the welfare costs are additive: the joint welfare cost (0.000537\%) essentially equals the sum of individual costs (0.000386\% $+$ 0.000151\% = 0.000537\%), reflecting the linear superposition property of the first-order perturbation solution. Under higher-order approximations, nonlinear interaction effects could generate amplification.

Second, intermittency dominates the welfare cost---contributing more than twice the battery price component (0.000386\% vs.\ 0.000151\%). This reflects the key role of AR(1) persistence ($\rho_{ren}=0.85$): once reliability degrades, households suffer prolonged consumption losses, generating a much larger welfare cost than a purely transitory disruption.
...
\textbf{First, the reliability penalty is a tangible macroeconomic friction.} A one-standard-deviation intermittency shock reduces grid utilization by 1.30\% and output by 0.55\% through the effective capital channel. The fixed proportional tax $\tau_{ss}=1.65\%$ creates a constant wedge that shifts the steady state (reducing $K_p$ by 28\% vs.\ the lump-sum benchmark) but does not amplify the dynamic shock response. Variance decomposition confirms that intermittency shocks account for 96.8\% of output volatility, with battery-price shocks contributing 4\%. The dominance of intermittency reflects its AR(1) persistence ($\rho_{ren}=0.85$), which generates sustained multi-quarter reliability degradation. Unlike standard productivity shocks, this penalty is structural and predictable---it arises from the physics of the grid, not from exogenous technological regress.
...
First, the \textit{reliability penalty with distortionary taxation}: intermittency shocks reduce grid utilization by 1.30\%, generating output losses of 0.55\%. The fixed proportional tax $\tau_{ss}=1.65\%$ creates a constant wedge on capital returns and labor income, shifting the steady state rather than amplifying shocks dynamically. Variance decomposition confirms that intermittency shocks account for 96.8\% of output volatility, with the remaining 4\% attributable to global battery-price shocks.
