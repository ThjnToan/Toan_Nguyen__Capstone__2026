...
\begin{figure}[H]
    ...
    Without these reforms, the 20\% welfare cost amplification identified in the counterfactual analysis (and 14\% for partial dampening at $\chi = 0.3$) represents a quantifiable cost of regulatory inertia.
...
\begin{equation}
V_t = (u_t K_{p,t-1})^\alpha L_t^{1-\alpha}
...
\noindent The result follows from the strict supermodularity of the CES aggregator when inputs are complements ($\rho < 1$). The formal proof and a quantitative characterization of the superadditivity gap are provided in Appendix~\ref{app:proof_superadditivity}. Table~\ref{tab:ces_complementarity} evaluates the gap numerically under the baseline calibration.
...
\begin{equation}
\tau_{ss} = \frac{I_{grid,ss}}{Y_{ss}} = \frac{\delta_g K_{g,ss}}{Y_{ss}} \approx 0.0165
\label{eq:gov_budget}
\end{equation}
...
\begin{enumerate}
    ...
    \item Proportional tax rate: $\tau_{ss} = I_{grid,ss}/Y_{ss} = \delta_g K_{g,ss}/Y_{ss} \approx 0.0165$ (1.65\%). This calibrated (fixed) tax rate is small but distortionary, driving a permanent wedge between gross and net factor returns.
...
\paragraph{Channel 3: Reliability Gap → Scarcity-Induced Innovation → Productivity Gain.}
...
The CES weight $\mu$ is calibrated to match the battery share of total planned energy infrastructure investment from PDP8. The investment-share interpretation of $\mu$ is: in equilibrium, the fraction of total energy investment directed to battery storage (relative to grid transmission) should reflect $\mu$. From the annual investment flows above:
\begin{equation}
\mu \approx \frac{I_{bat,\text{annual}}}{I_{bat,\text{annual}} + I_{grid,\text{annual}}} = \frac{0.24}{0.24 + 1.49} \approx 0.14
\label{eq:mu_calibration}
\end{equation}
I round this to $\mu = 0.16$ to account for the projected acceleration of battery investment under PDP8's storage mandate for 2026--2030 (Ministry of Industry and Trade, 2023). This reflects the relatively small but growing role of battery storage in Vietnam's current energy infrastructure relative to the dominant transmission network.\footnote{Note that in the model's investment-rule equilibrium, the steady-state rental rates on battery capital ($\approx 2.04$) and grid capital ($\approx 0.12$) are not equalized, since investment rules are reduced-form rather than derived from CES cost minimization. The parameter $\mu$ therefore acts as a weighting coefficient in the CES flexibility aggregator that governs substitution margins around the steady state, not as an optimality condition pin.}
...
\begin{itemize}
    \item $\psi = 2.0$: Sensitivity parameter governing how rapidly reliability degrades when the flexibility-to-volatility ratio declines. Calibrated by moment-matching: the value $\psi = 2.0$ is selected so that the steady-state utilization $u_{ss} = 0.97$ is achieved given the PDP8-implied flexibility capital stock ratio, consistent with EVN's reported grid availability of 97--98\% under normal operating conditions \citep{evn2023}. Sensitivity analysis confirms that qualitative results are robust for $\psi \in [1.5, 3.0]$ (see Section~\ref{sec:calibration}). Higher $\psi$ implies steeper reliability penalties from flexibility shortfalls; lower $\psi$ reduces the welfare cost magnitude but preserves all qualitative mechanisms.
...
The grid investment implementation shock has AR(1) persistence $\rho_I = 0.70$ (quarterly) and standard deviation $\sigma_I = 0.05$. The persistence $\rho_I = 0.70$ captures multi-quarter project delays that are endemic to large infrastructure programs in Vietnam: the World Bank (2022) documents average implementation delays of 2--3 years for energy sector projects, corresponding to a quarterly half-life of approximately $\ln(0.5)/\ln(0.70) \approx 1.9$ quarters in the AR(1) specification. The volatility $\sigma_I = 0.05$ (20\% annualized) is calibrated to match the coefficient of variation of annual public energy investment disbursements observed in Vietnam's State Budget Execution Reports (MoF 2018--2023), which show year-to-year variation of approximately 15--25\% around planned allocations due to budget sequestration, procurement delays, and contractor capacity constraints. As a result, implementation shocks account for 82\% of grid investment variance in the model --- consistent with the view that public energy investment in Vietnam is primarily constrained by execution capacity rather than the reliability signal itself.

\subsection{External Sector}

The world interest rate is set to $\bar{r} = 1/\beta - 1 = 0.0101$ (quarterly), ensuring consistency with the household's Euler equation in steady state. The discount factor $\beta = 0.99$ is matched to the average real yield on 10-year Vietnamese Government Bonds (VGBs) of approximately 4\% annually (GSO 2024); this is below Vietnam's average lending rate (8--12\% nominal) but appropriate for a model calibrated to the government's intertemporal trade-off rather than to private borrowing costs.

The debt-elastic premium parameter is set to $\phi_b = 0.039$, estimated from World Bank Vietnam real interest rate and external debt/GNI data (1995--2022) as the real-data-anchored value $\hat{\phi}_b = 0.039$. This estimate is above the standard \citet{schmittgrohe2003} value of 0.000742, consistent with Vietnam's partial capital-account integration. Vietnam maintains restrictions on cross-border capital flows under the Law on Foreign Exchange (2005, amended 2013), limits on offshore bond issuance by domestic firms, and a managed exchange rate regime that constrains the current account adjustment mechanism. Together, these features imply that the effective cost of international borrowing rises more steeply with the debt position than in fully open economies.\footnote{The estimate $\hat{\phi}_b = 0.039$ is obtained from the real-data estimator $\hat{\phi}_b = \max\!\left(0,\, \sum x_t y_t / \sum x_t^2\right)$ where $y_t = r_t - \bar{r}$ (World Bank Vietnam real lending rate minus its sample mean) and $x_t = \exp(d_t - \bar{d})-1$ (external debt/GNI deviation from sample mean), using $n=28$ overlapping annual observations. Sensitivity analysis confirms that results are qualitatively robust across $\phi_b \in \{0.001, 0.039, 0.10\}$: welfare costs remain in the range 0.0004\%--0.0006\%, the B-K conditions are satisfied throughout, and the key FEVD rankings (intermittency dominates output, battery prices dominate $I_{bat}$) are preserved.} The steady-state net foreign asset position is set to $\bar{B}^* = 0$ (balanced trade), a simplifying assumption that allows the model to focus on the transition dynamics of the energy sector rather than long-run external imbalances.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c l p{6.5cm}}
        ...
        Debt Premium Elast. & $\phi_b$ & 0.039 & Real-data estimate (WB VN real rate \& external debt/GNI, 1995--2022); reflects Vietnam's partial capital-account integration \\
        SS Net Foreign Assets & $\bar{B}^*$ & 0 & Balanced trade normalization \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
    \parbox{\textwidth}{\footnotesize \textit{Note:} Depreciation rates listed as quarterly values with annual equivalents in parentheses. Battery Weight ($\mu$) is calibrated to the battery share of total PDP8 energy infrastructure investment. $\phi_b$ is elevated above the \citet{schmittgrohe2003} standard to reflect Vietnam's capital-account restrictions; see Table~\ref{tab:phi_b_sensitivity} for sensitivity. All calibration targets documented in Section~\ref{sec:calibration}.}
    \parbox{\textwidth}{\footnotesize \textit{Note:} Depreciation rates listed as quarterly values with annual equivalents in parentheses. Battery Weight ($\mu$) is calibrated to the battery share of total PDP8 energy infrastructure investment. $\phi_b = 0.039$ is estimated from World Bank Vietnam real rate and external debt/GNI data (1995--2022); see Table~\ref{tab:phi_b_sensitivity} for sensitivity. All calibration targets documented in Section~\ref{sec:calibration}.}
\end{table}
...
The debt-elastic premium $\phi_b = 0.039$ has been calibrated from World Bank Vietnam data; nevertheless, a robustness check across the plausible range is warranted. Table~\ref{tab:phi_b_sensitivity} presents the sensitivity of key results to three values.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c}
        ...
        & (SGU standard) & (Data-Anchored Baseline) & (Strong restrictions) \\
        \midrule
        Welfare cost $\Lambda$ (baseline, $\chi=1$) & $0.000448\%$ & $0.000569\%$ & $0.000572\%$ \\
        $I_p$ impact (intermitten.\ shock) & $-21.8\%$ & $-3.5\%$ & $-2.6\%$ \\
        $B^*$ impact (intermitten.\ shock) & Surplus ($+0.048$) & Surplus ($+0.003$) & Surplus ($+0.001$) \\
        B-K conditions & Satisfied & Satisfied & Satisfied \\
        Qualitative FEVD rankings & Unchanged & Unchanged & Unchanged \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
    \parbox{\textwidth}{\footnotesize \textit{Note:} All values are computed from Dynare first-order perturbation at the calibrated steady state. The key qualitative findings---intermittency dominates output variance, battery prices dominate $I_{bat}$ variance, welfare rises monotonically in $\chi$---are robust across all three values. The data-anchored baseline $\phi_b = 0.039$ eliminates the implausibly large $I_p$ volatility ($-21.8\%$) generated by the SGU-standard value and is preferred for Vietnam.}
\end{table}
...
The debt-elastic premium ensures that the net foreign asset process is stationary: when the economy borrows ($B^*_t < 0$), the interest rate rises, discouraging further borrowing and stabilizing the external position. The value $\phi_b = 0.039$ is estimated from World Bank Vietnam real interest rate and external debt/GNI data (1995--2022), reflecting Vietnam's partial capital-account integration: capital controls, a managed exchange rate, and underdeveloped bond markets all limit the household's ability to borrow freely at the world interest rate. A higher $\phi_b$ reduces consumption smoothing via international borrowing, ensuring that intermittency shocks propagate more fully into domestic consumption and welfare.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        \midrule
        Output ($Y$) & 0.861 & Normalized \\
        Consumption ($C$) & 0.630 & 73.2\% of output \\
        Labor ($L$) & 0.330 & Normalized hours \\
        Productive Capital ($K_p$) & 8.448 & Production capacity \\
        Battery Capital ($K_b$) & 0.190 & Private storage capacity \\
        Grid Capital ($K_g$) & 1.140 & Public transmission infrastructure \\
        Value Added ($V$) & 1.016 & $(u \cdot K_p)^\alpha L^{1-\alpha}$ \\
        ...
        Productive Investment ($I_p$) & 0.211 & $= \delta_p \cdot K_p$ \\
        Battery Investment ($I_{bat}$) & 0.0057 & $= \delta_b \cdot K_b$ \\
        Grid Investment ($I_{grid}$) & 0.0143 & $= \delta_g \cdot K_g$ \\
        Tax Rate ($\tau$) & 0.0165 & Grid investment / output \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
    \parbox{\textwidth}{\footnotesize \textit{Note:} The battery-to-grid capital ratio ($K_b / K_g \approx 0.167$) is calibrated directly from PDP8 investment allocations. The consumption share of 73.2\% reflects the small open economy specification with balanced trade in steady state ($B^*_{ss} = 0$); the labor disutility weight $\varphi$ is calibrated endogenously to clear the labor market at $L_{ss} = 0.33$. The steady-state tax rate $\tau_{ss} = 1.65\%$ is endogenously determined from the proportional tax rule $\tau_t = I_{grid,t}/Y_t$.}
\end{table}
...
\begin{figure}[H]
    ...
    \parbox{0.95\textwidth}{\footnotesize \textit{Note:} Impulse responses to a one standard deviation intermittency shock ($\sigma_{ren} = 0.12$). Output $Y_t$, consumption $C_t$, productive investment $I_{p,t}$, and value added $V_t$ in level deviations from steady state. Productive investment falls sharply ($-23.5\%$ relative to SS) on impact due to Euler equation amplification: the expected fall in future output reduces the after-tax return to capital, compressing the incentive to invest. Consumption and output decline modestly but persistently, governed by the AR(1) intermittency process ($\rho_{ren}=0.85$). The fixed proportional tax $\tau_{ss}=1.65\%$ creates a permanent wedge that shifts the steady state but does not amplify the dynamic response.}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c}
        ...
        \midrule
        \multicolumn{3}{l}{\textit{Agility ratio (level): $I_{bat}/I_{grid} = 0.40$; battery investment is dominated by global price shocks}} \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
...
The proportional tax is fixed at $\tau_{ss} = 1.65\%$ and does not respond dynamically to shocks. Its effect on the investment response operates through the steady-state channel: the tax wedge reduces the equilibrium capital stock to $K_{p,ss} = 8.45$, compared to approximately 11.7 under lump-sum taxation. With a smaller capital base, productive investment $I_{p,ss} = \delta_p K_{p,ss} = 0.211$ is also smaller. Consequently, the Euler equation amplification is larger in percentage terms: the expected fall in future output reduces the after-tax return to capital sharply, causing productive investment to fall by 23.5\% relative to its steady-state level on impact. This large percentage response reflects the high capital-output ratio ($K/Y \approx 9.8$) and low depreciation rate ($\delta_p = 2.5\%$), which together create a strong ``leverage'' amplification in the standard RBC Euler mechanism.
...
\begin{table}[H]
    ...
    \end{tabular}
    \vspace{0.2cm}
    \parbox{\textwidth}{\footnotesize \textit{Note:} All values are percentage deviations from steady state on impact ($t=0$) following a one standard deviation intermittency shock ($\sigma_{ren} = 0.12$). Computed from Dynare first-order perturbation of \texttt{vietnam\_dsge\_proptax.mod}. Variance shares from simulation-based FEVD (Table~\ref{tab:fevd}). The proportional tax rate $\tau_{ss}=1.65\%$ is fixed and does not respond dynamically to shocks. The large $I_p$ response reflects RBC Euler amplification: with capital-output ratio $K_p/Y \approx 9.8$ and $\delta_p = 2.5\%$, small persistent output falls generate large percentage investment swings. The deterministic steady state serves as the reference economy for the welfare comparisons in Section~\ref{sec:welfare}.}
\end{table}
...
The reference economy is the deterministic steady state described in Table~\ref{tab:steady_state}, where $C_{ss} = 0.630$, $u_{ss} = 0.97$, $\tau_{ss} = 1.65\%$, and all shocks are zero. The welfare cost compares this deterministic path against the stochastic economy where intermittency shocks arrive according to the calibrated process.
...
\noindent The proof is provided in Appendix~\ref{app:proof_chi}. Intuitively, a higher $\chi$ amplifies the scarcity-induced innovation response $\dot{A}_{bat}$ to any given reliability gap, which improves battery technology faster, restores flexibility sooner, and reduces the welfare cost of intermittency. Complete suppression ($\chi = 0$) eliminates the innovation channel entirely, forcing the economy to rely on costly capital accumulation alone---the highest-cost adjustment path.
...
\textbf{Fourth, the model's transmission channels are quantitatively validated.} The Dynare simulation confirms expected signs: output and reliability are highly correlated (0.986), and battery price shocks affect investment negatively ($\rho = -0.979$ between $P_{bat}$ and $I_{bat}$). The proportional tax is fixed, so its role appears entirely through steady-state comparisons: the constant wedge $\tau_{ss}=1.65\%$ shifts equilibrium quantities, not their dynamic comovement. These correlations validate the model's internal consistency.
...
\noindent The marginal product of labor in the model is derived through the chain rule across the nested production structure: $Y_t \to V_t \to L_t$. From the value-added function $V_t = (u_t K_{p,t-1})^\alpha L_t^{1-\alpha}$, the marginal product with respect to $L_t$ is $(1-\alpha)V_t/L_t$ (the predetermined capital $K_{p,t-1}$ is constant with respect to $L_t$). Passing this through the CES aggregator $Y_t = [(1-\omega_E)V_t^{s_E} + \omega_E \bar{E}^{s_E}]^{1/s_E}$, the chain rule gives $\partial Y_t/\partial L_t = (\partial Y_t / \partial V_t)\cdot(\partial V_t / \partial L_t)$. Firms in a competitive labor market equate the real wage $w_t$ to this marginal product:
\begin{equation*}
