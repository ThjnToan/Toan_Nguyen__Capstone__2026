...
\begin{equation}
C_t + I_{p,t} + P_{bat,t} I_{bat,t} + I_{grid,t} + B^*_t
=
Y_t + (1+r^*_{t-1}) B^*_{t-1}
\end{equation}

where $I_{p,t}$ is productive capital investment, $I_{bat,t}$ battery investment at world price $P_{bat,t}$, $I_{grid,t}$ public grid investment (following the rule specified in Section~\ref{sec:government}), and $B^*_t$ denotes net foreign asset holdings earning the country-specific interest rate $r^*_t$.
...
Grid investment is financed through lump-sum taxation, which is embedded in the resource constraint \eqref{eq:goods_market_clearing}. The government does not optimize; it follows the mechanical rule above, consistent with the reactive nature of infrastructure policy in developing economies.
...
The economy-wide resource constraint in the open economy is:
\begin{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{agility_gap_dynare.png}
    \caption{\textbf{The Reliability Valley: PDP8 Transition Path}}
    \label{fig:reliability_valley}
    \parbox{0.95\textwidth}{\footnotesize \textit{Note:} Left panel shows the policy-driven increase in renewable penetration (green line), modeled as a front-loaded S-curve reflecting PDP8 targets. Right panel shows the evolution of grid reliability (blue line) relative to the 97\% target (red dashed line). The Reliability Valley (orange vertical line) occurs at quarter 45 (year 11.2), when the gap between VRE deployment and flexibility capacity is largest. The red shaded region indicates periods when reliability falls below the policy target, requiring emergency interventions such as load shedding, industrial demand curtailment, or increased reliance on fossil backup generation.}
\end{figure}

\subsubsection{Valley Characteristics}

The Reliability Valley reaches its nadir at \textbf{quarter 45}, approximately 11.2 years into the transition. At this point, grid reliability drops to \textbf{91.98\%}---a decline of 5.17 percentage points below the 97\% target. To contextualize this magnitude: a reliability rate of 92\% implies that the grid fails to meet demand for approximately 700 hours per year, or roughly 8\% of all hours. For comparison, the 2021 Texas winter storm reduced grid reliability to approximately 88--90\% over a multi-day period, triggering widespread economic disruption and an estimated \$130 billion in damages \citep{callaway2018}.

While Vietnam's valley is less severe than the Texas event, it represents a \textit{sustained} period of degraded performance rather than a one-time crisis. The valley persists for approximately 10--15 quarters, during which reliability remains below target. This prolonged instability has several economic consequences:
