...
\paragraph{Euler equation for capital.} Differentiating the Lagrangian with respect to $K_{p,t+1}$ and $C_t$, and noting that the marginal product of capital in the production function is $\partial Y_{t+1} / \partial K_{p,t} = \alpha Y_{t+1}/K_{p,t}$ (from the Cobb-Douglas value-added structure in equation~\ref{eq:value_added} below), the intertemporal optimality condition equates the marginal cost of investing one unit of consumption today with the discounted marginal benefit of the return tomorrow:\footnote{The full step-by-step Lagrangian derivation of this condition is provided in Appendix~\ref{app:deriv_euler}.}
\begin{equation}
\frac{1}{C_t} = \beta \mathbb{E}_t \left[ \frac{\alpha Y_{t+1}/K_{p,t+1} + 1 - \delta_p}{C_{t+1}} \right]
\label{eq:euler}
\end{equation}

The household invests in capital until the cost of forgoing one unit of consumption today ($1/C_t$) equals the expected discounted benefit of the gross return---marginal product plus undepreciated capital---valued at tomorrow's marginal utility ($1/C_{t+1}$).

\paragraph{Labor supply.} Differentiating with respect to $L_t$ and equating the marginal disutility of labor ($L_t^{\sigma_L}$) with the marginal utility of the real wage ($w_t/C_t$), and substituting the competitive wage $w_t = (1-\alpha)V_t/L_t$ from the firm's profit maximization:\footnote{The full step-by-step Lagrangian derivation of this condition, including the substitution of the competitive wage, is provided in Appendix~\ref{app:deriv_labor}.}
\begin{equation}
(1-\alpha) V_t = \varphi \, C_t \, L_t^{1+\sigma_L}
\label{eq:labor_supply}
\end{equation}

The labor share of value-added (left side) equals the household's required compensation for working, which is increasing in consumption (higher income households demand more leisure) and steeply increasing in hours worked (via the exponent $1+\sigma_L$). The parameter $\varphi$ is the labor disutility weight, calibrated endogenously to match steady-state labor hours of $L_{ss} = 0.33$.
...
Substituting the government budget constraint \eqref{eq:gov_budget} into the household budget constraint \eqref{eq:hh_budget} eliminates $T_t$ and yields the economy-wide resource constraint. Because $T_t$ is lump-sum, the household cannot optimize around it, and the consolidated allocation is identical to one in which the household directly pays for grid investment---an instance of Ricardian equivalence. On the income side, substituting $w_t L_t + R_t K_{p,t-1} = V_t$ and noting that in the model's calibrated equilibrium the energy share $\omega_E = 0.045$ is small, so $V_t \approx Y_t$, the consolidated income term simplifies to $Y_t$.\footnote{More precisely, $(1-\alpha)V_t + \alpha Y_t = Y_t - \alpha(Y_t - V_t)$. The gap $Y_t - V_t$ represents the contribution of the exogenous energy endowment $\bar{E}$ to gross output; with $\omega_E = 0.045$, this gap is quantitatively small. For the resource constraint, it is absorbed into $Y_t$ directly, with the energy endowment treated as a free domestic resource.} The economy-wide resource constraint in the open economy is:
\begin{equation}
...
Consumption today equals expected consumption tomorrow minus a correction for expected changes in the return on capital. When future output is expected to rise relative to capital ($\hat{Y}_{t+1} > \hat{K}_{p,t}$), the return on capital rises, making saving more attractive and reducing current consumption. The coefficient $\alpha Y_{ss}/(K_{p,ss} R_{ss})$ is the intertemporal elasticity of substitution scaled by the steady-state capital share, where $R_{ss} = \alpha Y_{ss}/K_{p,ss} + 1 - \delta_p = 1/\beta$ is the gross return on capital.
...
A 1\% increase in value-added raises the real wage by 1\%, which must be absorbed by either higher consumption ($\hat{C}_t$) or reduced labor supply ($\hat{L}_t$). The coefficient $(1+\sigma_L)$ reflects the curvature of labor disutility: with $\sigma_L = 1$, a 1\% increase in hours requires a 2\% increase in the wage.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c}
        ...
        Partial dampening ($\chi = 0.3$) & 0.000063\% & +17\% \\
...
The results demonstrate that signal attenuation has quantitatively important welfare consequences. Moving from full transmission to complete suppression increases the welfare cost of intermittency by 35\%, from 0.000054\% to 0.000073\% of steady-state consumption. This increase arises because the scarcity-induced innovation channel is deactivated when $\chi = 0$: battery technology fails to improve in response to reliability shortfalls, forcing the economy to rely solely on costly capital accumulation to restore grid stability.
...
Third, \textit{scarcity-induced technological change}: technological progress in battery storage responds endogenously to the gap between target and actual reliability, creating a self-correcting mechanism that gradually increases the productivity of flexibility provision.
...
\log(C_t) - \varphi \frac{L_t^{1+\sigma_L}}{1+\sigma_L} \\
&\quad + \lambda_t \Big[Y_t + (1+r^*_{t-1})B^*_{t-1} - C_t 
- K_{p,t+1} + (1-\delta_p)K_{p,t}
- P_{bat,t}I_{bat,t} - B^*_t - T_t\Big]
\Bigg\}
\end{align*}
...
\left(\frac{\alpha Y_{t+1}}{K_{p,t+1}} + 1 - \delta_p\right)\right] = 0
\end{equation*}
...
This is exactly the capital Euler equation~\eqref{eq:euler}. It equates the marginal utility cost of saving (the left-hand side, $1/C_t$) with the expected discounted marginal utility benefit of the gross return on capital (the right-hand side). At the optimum, the household is indifferent at the margin between consuming an additional unit today and investing it in productive capital.
...
\begin{equation*}
\frac{\alpha Y_{ss}}{K_{p,ss}} = \frac{1}{\beta} - 1 + \delta_p
\end{equation*}
which pins down the steady-state capital-to-output ratio as $K_{p,ss}/Y_{ss} = \alpha\beta/(1 - \beta(1-\delta_p))$.
...
Using the same Lagrangian as in Appendix~\ref{app:deriv_euler}, differentiate with respect to $L_t$. Labor enters utility directly (disutility) and indirectly through output $Y_t$ (which affects the budget constraint via income):
\begin{equation*}
\frac{\partial \mathcal{L}}{\partial L_t}
= \beta^t\left[-\varphi L_t^{\sigma_L} + \lambda_t \frac{\partial Y_t}{\partial L_t}\right] = 0
\end{equation*}
...
\begin{equation*}
-\varphi L_t^{\sigma_L} + \lambda_t \, w_t = 0
\quad \Longrightarrow \quad
\varphi L_t^{\sigma_L} = \lambda_t \, w_t
\end{equation*}
...
\begin{equation*}
\varphi L_t^{\sigma_L} = \frac{w_t}{C_t}
\end{equation*}
...
\begin{equation*}
L_t^{\sigma_L} = \frac{1}{C_t} \cdot \frac{(1-\alpha)\,V_t}{L_t}
\end{equation*}

Multiplying both sides by $C_t \, L_t$:
\begin{equation*}
\varphi \, C_t \, L_t^{1+\sigma_L} = (1-\alpha)\,V_t
\end{equation*}
...
\end{equation*}
This scaling parameter is suppressed for clarity in the derivation above (equivalently, set $\varphi = 1$ during the derivation and introduce it by calibration). Re-inserting $\varphi$ into the labor FOC:
\begin{equation*}
\varphi\, L_t^{\sigma_L} = \frac{w_t}{C_t}
\end{equation*}
Following the same steps yields:
\begin{equation*}
(1-\alpha)\,V_t = \varphi\,C_t\,L_t^{1+\sigma_L}
\end{equation*}
...
The left-hand side, $(1-\alpha)V_t$, is total labor income (the wage bill): it is the household's revenue from supplying labor to the competitive firm, equal to the labor share $(1-\alpha)$ of value-added $V_t$. The right-hand side, $\varphi\,C_t\,L_t^{1+\sigma_L}$, is the household's reservation wage scaled by consumption: it equals the ratio of the marginal disutility of labor ($\varphi L_t^{\sigma_L}$) to the marginal utility of consumption ($1/C_t$), which is the standard intratemporal optimality condition ``marginal rate of substitution = real wage.''

In steady state, this condition, together with the resource constraint and the capital Euler equation, pins down steady-state hours $L_{ss} = 0.33$ (calibrated target) and the disutility weight $\varphi$ (calibrated endogenously). When $\sigma_L = 1$ (baseline calibration), the condition becomes $(1-\alpha)V_t = \varphi\,C_t\,L_t^2$, implying a unit elasticity of labor supply with respect to the real wage.
